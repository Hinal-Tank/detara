...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className="fixed top-0 right-0 z-[301] h-full w-full max-w-[420px] bg-bg shadow-2xl flex flex-col"
      ...
      >
        ...
        {/* Items */}
        <div
          className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-4 scrollbar-none"
          style={{ WebkitOverflowScrolling: 'touch' }}
        >
          {items?.length === 0 ? (
            <div className="text-center py-16">
              <p className="font-serif text-lg font-light text-muted mb-4">
                Your cart is empty.
              </p>
            ...
            items?.map((item) => (
              <div
                key={item?.id}
                className="flex gap-3 p-3 md:p-4 bg-bg-warm border border-[rgba(28,25,23,0.06)]"
              >
                <div
                  className="relative w-18 h-18 md:w-20 md:h-20 flex-shrink-0 bg-[#EAE2D8] overflow-hidden"
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-serif text-sm font-light text-foreground mb-1 leading-snug">
                    {item?.name}
                  </h3>

                  <p
                    className="label-caps text-muted mb-2"
                    style={{ fontSize: '8px' }}
                  >
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center border border-[rgba(28,25,23,0.12)]">
                      ...
                      <span className="w-6 text-center text-xs font-light text-foreground">
                        {item?.quantity}
                      </span>
